...
--GRANT SELECT  ON Ventas.vw_Resumen_Ventas_Por_Sede          TO usr_nivel4; --no
--GRANT SELECT  ON Ventas.vw_Top_Productos_Global             TO usr_nivel4; --no
...
--GRANT SELECT  ON Configuracion.vw_Detalle_Sincronizacion    TO usr_nivel4; --innecesario
...
IF NOT EXISTS (SELECT 1 FROM Seguridad.Usuario WHERE Username = 'cliente@test.com')
BEGIN
    INSERT INTO Seguridad.Usuario (id_persona, id_sede, Username, Password, Nivel_acceso, Estado)
    VALUES (3, 1, 'cliente@test.com', 'Abc123!', 1, 1);
END
GO