-- Bitácora: usar usuario de sesión; si no hay, preferir dueño (nivel 4) en lugar de MIN(id) (Mamani)
USE TiendaRopa;
GO
...
BEGIN
    ...
    IF @id_usuario IS NULL
       OR NOT EXISTS (SELECT 1 FROM Seguridad.Usuario WHERE id_usuario = @id_usuario AND Estado = 1)
    BEGIN
        SELECT TOP 1 @id_usuario = id_usuario
        FROM Seguridad.Usuario
        WHERE Nivel_acceso = 4 AND Estado = 1
...
PRINT 'OK: Persona.trg_Bitacora_Persona — usa SESSION_CONTEXT o dueño (nivel 4)';
